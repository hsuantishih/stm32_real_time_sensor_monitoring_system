...
static void I2C_LCD_EnPulse(uint8_t I2C_LCD_InstanceIndex, uint8_t DATA)
{
	...
	HAL_Delay(0.002); // enable pulse must be >450ns

    I2C_LCD_ExpanderWrite(I2C_LCD_InstanceIndex, (DATA & ~EN)); // En low
    HAL_Delay(0.05); // commands need > 37us to settle
}
...
void I2C_LCD_Init(uint8_t I2C_LCD_InstanceIndex)
{
    ...
    HAL_Delay(4500);  // Delay > 4.1ms
    I2C_LCD_Cmd(I2C_LCD_InstanceIndex, 0x30);
    HAL_Delay(4500);  // Delay > 4.1ms
    I2C_LCD_Cmd(I2C_LCD_InstanceIndex, 0x30);
    HAL_Delay(0.15);  // Delay > 4.1ms
...
void mylcdprintf_ext(uint8_t I2C_LCD_InstanceIndex, uint8_t Col, uint8_t Row, char* Str, bool ClearScreen)
{
    if (ClearScreen) {
        I2C_LCD_Clear(I2C_LCD_InstanceIndex);
    }
    I2C_LCD_SetCursor(I2C_LCD_InstanceIndex, Col, Row);
    I2C_LCD_WriteString(I2C_LCD_InstanceIndex, Str);
}